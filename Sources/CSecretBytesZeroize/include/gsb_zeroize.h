...
/// musl), or a `volatile` byte loop as a last resort — which is also the path
/// Bionic takes, since it declares and links `explicit_bzero` on no NDK API
/// level (checked 21-35). No-ops when `ptr` is NULL or `len` is 0.
